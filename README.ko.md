...
| 반응 민감도(걷기/대시/회전/점프/웅크리기/보폭/걷기·대시 지속 판정) | 각각 독립적으로 "약함"~"강함"(보폭·걷기/대시 지속 판정만 "좁음"~"넓음") 슬라이더로 조정 가능(전진/후진에는 영향 없음). 중앙(기본값)은 기존 판정 기준 그대로 유지됩니다. 대시/회전/점프/웅크리기는 "약함"을 최소(0)로 하면 판정과 무관하게 완전히 비활성화됩니다 — 자세히는 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
