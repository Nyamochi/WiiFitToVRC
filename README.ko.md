...
3. 앱은 실행되는 즉시 자동으로 검색을 시작합니다. Wii 밸런스 보드의 배터리 케이스 안에 있는 **SYNC** 버튼만 누르면 자동으로 연결됩니다(연결 버튼을 누를 필요가 없습니다). 한 번 페어링한 보드라면 다음부터는 배터리 케이스를 열어 SYNC 버튼을 누를 필요 없이, 그냥 전원을 켜기만 해도 자동으로 다시 연결됩니다.
4. 연결이 완료되면 보정도 자동으로 시작됩니다. "기기를 바닥에 놓아주세요"라는 메시지가 표시된 후 5초 뒤 자동으로 보정이 시작되므로, 그때까지 보드를 바닥에 놓고 아무것도 올려두지 마세요.
5. 보정이 끝나면 보드에 올라가 잠시 평소처럼 서 있으면 준비가 끝납니다. 이제 VRChat을 실행하고 보드 위에서 체중을 이동하면 걸을 수 있습니다.
