...
| 反應靈敏度(走路/衝刺/轉向/跳躍/蹲下/步幅/行走衝刺持續判定/達到持續判定的步數) | 均可分別獨立用「弱」~「強」(步幅、行走/衝刺持續判定為「窄」~「寬」,達到持續判定的步數為1~10的步數)滑桿調整(不影響前進/後退)。中間(預設值,達到持續判定的步數為5)保持原有判定基準不變。衝刺/轉向/跳躍/蹲下這四項若將「弱」調到最小(0),則無論判定結果為何都完全不會輸出按鍵。轉向不會持續,始終每次只動作一步 — 詳見 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
| 轉向方式 | 可在「長按」(向左右傾斜並保持)與「踏步」(對角面板交替踩踏,預設)之間切換轉向判定邏輯 |
| 衝刺輸入方式 | 可在鍵盤/鍵盤+滑鼠模式下,將衝刺傳送方式在「組合鍵」(修飾鍵+前進鍵,預設)與「按鍵連按兩次」(按一次前進鍵後再長按)之間切換 |
| 轉向速度 | 各輸出方式的絕對值不同,因此每次切換輸出方式單選按鈕時,滑桿的範圍與數值都會自動切換。左右共用同一數值。選擇「鍵盤(Q/E轉向)」時滑桿會消失,顯示「無設定項」 |
