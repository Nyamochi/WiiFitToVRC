...
1. 開啟 Windows 的藍牙功能(如果沒有藍牙天線,請另外準備 USB 藍牙轉接器等)。
2. 點擊本儲存庫頂部的 `WiiFitToVRC.exe` 下載(無需安裝)。
3. 雙擊下載的檔案即可執行。
4. 應用程式啟動後會自動開始搜尋。只需按下 Wii 平衡板電池盒內的 **SYNC** 按鈕即可自動連接(無需點擊連接按鈕)。由於規格限制,POWER 按鈕無法用於此用途,因此每次連接都需要按 SYNC 按鈕。
5. 連接完成後,校準也會自動開始。畫面會顯示「請將裝置放在地板上」,5秒後自動開始校準,請在此之前把平衡板放在地板上並保持空載。
6. 校準完成後,站上平衡板像平常一樣站立片刻即可完成準備。之後啟動 VRChat,在平衡板上移動體重即可行走。
...
跳躍動作超出了 Wii 平衡板原本的規格範圍,請注意周遭環境,避免損壞裝置或地板,
**體重超過 100kg 的使用者請不要在平衡板上跳躍**。即使是輕微的跳躍也會被偵測到。
...
- **四種輸出模式**:
  ...
  - 虛擬手把 — 適用於會拒絕 SendInput 合成鍵盤/滑鼠輸入的遊戲(包括 VRChat)。詳情請參見 [docs/VRCHAT_INPUT.md](docs/VRCHAT_INPUT.md)(英文)。
- 按鍵綁定/手把分配、轉向速度、重量閾值、各類時間參數等均可在應用程式內設定視窗中細部調整。
- 主畫面右下角即時顯示行走距離(走路・衝刺各自以公尺為單位)和步數。僅統計從啟動到結束的期間,不會儲存。
- 多語言介面:自動偵測 Windows 顯示語言,內建日文、英文、簡體/繁體中文、韓文、法文、德文、義大利文。
- **遊玩姿勢(站立/坐姿)切換** — 可在主畫面左下角切換。選擇坐姿後會自動調整觸發所需的重量閾值和休眠/喚醒所需秒數,前進/後退、跳躍、蹲下的判定邏輯也會切換為適合坐姿的版本(詳見 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md))。
- 啟動時自動檢查 GitHub 上的最新版本,如有更新會彈出通知(附更新內容和儲存庫連結,點擊 OK 即可關閉,不會強制操作)。
...
| 設定項目 | 作用 |
|---|---|
| 輸出方式 | 鍵盤 / 鍵盤+滑鼠 / VRChat OSC / 虛擬手把(詳見 [docs/VRCHAT_INPUT.md](docs/VRCHAT_INPUT.md)) |
| 語言 | 介面顯示語言,也可設為自動跟隨 Windows 設定 |
| 反應靈敏度(走路/衝刺/轉向/跳躍/蹲下/步幅/行走衝刺持續判定/達到持續判定的步數) | 均可分別獨立用「遲鈍」~「敏感」(步幅、行走/衝刺持續判定為「窄」~「寬」,達到持續判定的步數為1~15的步數)滑桿調整(不影響前進/後退)。中間(預設值,達到持續判定的步數為7)保持原有判定基準不變。衝刺/轉向/跳躍/蹲下這四項若將「遲鈍」調到最小(0),則無論判定結果為何都完全不會輸出按鍵。轉向不會持續,始終每次只動作一步 — 詳見 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
| 轉向方式 | 可在「長按」(向左右傾斜並保持)與「踏步」(對角面板交替踩踏,預設)之間切換轉向判定邏輯 |
| 衝刺輸入方式 | 可在鍵盤/鍵盤+滑鼠模式下,將衝刺傳送方式在「組合鍵」(修飾鍵+前進鍵,預設)與「按鍵連按兩次」(按一次前進鍵後再長按)之間切換 |
| 轉向速度 | 各輸出方式的絕對值不同,因此每次切換輸出方式單選按鈕時,滑桿的範圍與數值都會自動切換。左右共用同一數值。選擇「鍵盤(Q/E轉向)」「OSC功能」時滑桿會消失,顯示「無設定項」(OSC的轉向是`/input/LookLeft`・`/input/LookRight`這種單純的按鈕按下,沒有強度概念) |
| 轉向長度 | 僅在 OSC/虛擬控制器輸出下,一次轉向操作實際持續輸出的長度(預設1秒,窄~寬可調)。當 VRChat 的 OSC 輸入等因轉向脈衝過短而無反應時可以延長。兩種輸出方式共用同一數值,選擇鍵盤/鍵盤+滑鼠時該項不會顯示 |
...
| 手把分頁 | 虛擬手把模式下各動作對應的按鈕(轉向速度請於「一般」分頁的「轉向速度」中設定) |
