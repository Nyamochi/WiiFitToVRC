...
2. 雙擊下載的檔案即可執行。
3. 按下 Wii 平衡板電池盒內的 **SYNC** 按鈕,然後點擊應用程式中的 **接続(連接)** 按鈕。
4. 依照畫面提示操作(**キャリブレーション(校準)** → 走下平衡板等待 → 重新站上平衡板等待)即可完成準備。之後啟動 VRChat,在平衡板上移動體重即可行走。

提示:若要確認連接是否正常,可以開啟記事本並踩上平衡板 —— 如果輸入了 w/s/a/d,表示連接正常。若在 VR 模式等情況下遊戲仍然沒有反應,請嘗試在設定中將輸出方式切換為 VRChat OSC。

更詳細的步驟請參見下方的「快速開始」;若無法正常運作,可參考 [docs](docs/) 資料夾(英文)中各功能的詳細說明。
...
- 支援 HID 裝置的藍牙轉接器
- 若使用虛擬手把輸出模式:需要 [ViGEmBus](https://github.com/nefarius/ViGEmBus/releases)(這是一個真實的核心驅動程式 — 本應用程式無法自動為您安裝,請自行下載並安裝)

## 快速開始

1. 從本儲存庫根目錄下載 `WiiFitToVRC.exe` 並執行(自我完備建置,無需安裝 .NET 執行階段)。
2. 按下平衡板電池盒內的 **SYNC** 按鈕,然後點擊應用程式中的 **接続(連接)** 按鈕。
3. 連接後點擊 **キャリブレーション(校準)**,走下平衡板進行 10 秒鐘的感測器校準。
4. 重新站上平衡板並像平常一樣站立一段時間。在動作偵測開始前,需要一段持續靜止的時間來學習基準體重(在此之前狀態列會顯示「體重校準中」)。
5. 開啟 **設定** 視窗選擇輸出模式,並調整按鍵綁定/靈敏度。
