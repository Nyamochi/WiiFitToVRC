...
2. 雙擊下載的檔案即可執行。
3. 應用程式啟動後會自動開始搜尋。只需按下 Wii 平衡板電池盒內的 **SYNC** 按鈕即可自動連接(無需點擊連接按鈕)。由於規格限制,POWER 按鈕無法用於此用途,因此每次連接都需要按 SYNC 按鈕。
4. 連接完成後,校準也會自動開始。畫面會顯示「請將裝置放在地板上」,5秒後自動開始校準,請在此之前把平衡板放在地板上並保持空載。
5. 校準完成後,站上平衡板像平常一樣站立片刻即可完成準備。之後啟動 VRChat,在平衡板上移動體重即可行走。
