...
| 反應靈敏度(走路/衝刺/轉向/跳躍/蹲下/步幅/行走衝刺持續判定) | 均可分別獨立用「弱」~「強」(步幅、行走/衝刺持續判定為「窄」~「寬」)滑桿調整(不影響前進/後退)。中間(預設值)保持原有判定基準不變。衝刺/轉向/跳躍/蹲下這四項若將「弱」調到最小(0),則無論判定結果為何都完全不會輸出按鍵 — 詳見 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
