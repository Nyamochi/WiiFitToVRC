...
3. アプリは起動した時点で自動的に検索を始めます。Wiiバランスボードの電池ボックスにある **SYNC** ボタンを押すだけで、自動的に接続されます(接続ボタンを押す必要はありません)。なお、一度ペアリングしたボードであれば、次回以降は電池ボックスを開けてSYNCボタンを押さなくても、普通に電源を入れるだけで自動的に再接続されます。
4. 接続が完了すると、キャリブレーションも自動的に始まります。「機器を床に置いてください」と表示されてから5秒後に自動で開始されるので、それまでにボードを床に置いて何も乗せない状態にしてください。
5. キャリブレーション完了後、ボードに乗ってしばらく普通に立っていれば準備完了です。あとはVRChatを起動し、ボードの上で体重を移動すれば歩けます。
