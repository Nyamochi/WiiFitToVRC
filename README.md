...
| 反応のしやすさ(歩き/ダッシュ/旋回/ジャンプ/しゃがみ/歩幅) | それぞれ独立に「弱い」〜「強い」(歩幅のみ「狭い」〜「広い」)のスライダーで調整可能(前進/後進には影響しません)。既定値(中央)は従来の判定基準のまま。歩き・歩幅を除く4項目は「弱い」を最小(0)にすると、判定に関係なく一切キー出力されなくなります — 詳細は[docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
