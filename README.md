...
| 旋回の速さ | 出力方式(キーボード+マウス/OSC/仮想コントローラー)ごとに絶対値が異なるため、出力方式の選択を切り替えるたびにスライダーの範囲・値が自動的に切り替わります。左右共通の値。「キーボード(Q/Eで旋回)」選択時はスライダーが消え「設定なし」と表示されます |
