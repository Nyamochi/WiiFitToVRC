...
| 反応のしやすさ(歩き/ダッシュ/旋回/ジャンプ/しゃがみ/歩幅/歩行・ダッシュの継続判定/継続判定までの歩数) | それぞれ独立に「弱い」〜「強い」(歩幅・歩行/ダッシュの継続判定は「狭い」〜「広い」、継続判定までの歩数は1〜5の歩数)のスライダーで調整可能(前進/後進には影響しません)。既定値(中央、継続判定までの歩数は3)は従来の判定基準のまま。歩き・歩幅・歩行/ダッシュの継続判定・継続判定までの歩数を除く4項目は「弱い」を最小(0)にすると、判定に関係なく一切キー出力されなくなります — 詳細は[docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
