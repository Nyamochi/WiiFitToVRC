...
public static class Localizer
{
    ...
    private static readonly Dictionary<string, string[]> Table = new()
    {
        ...
        ["Caption_MovementMode"] = ["Movement method", "移動方法", "移动方式", "移動方式", "이동 방법", "Mode de déplacement", "Bewegungsart", "Modalità di movimento"],
        ["MovementMode_Footstep"] = ["Footstep", "足踏み", "踏步", "踏步", "발걸음", "Pas alternés", "Schrittwechsel", "Passo"],
        ["MovementMode_WeightShift"] = ["Weight shift", "体重移動", "体重转移", "體重轉移", "체중 이동", "Transfert de poids", "Gewichtsverlagerung", "Spostamento del peso"],
        ...
        ["Status_WeightCalibrating"] = ["Weight calibrating (please stand in the center of the board)...", "体重キャリブレーション中(パネルの中央に立っていてください)...", "体重校准中(请站在踏板中央)...", "體重校準中(請站在踏板中央)...", "체중 보정 중(보드 중앙에 서 있어 주세요)...", "Étalonnage du poids (tenez-vous au centre de la planche)...", "Gewichtskalibrierung läuft (bitte in der Mitte des Boards stehen)...", "Calibrazione del peso in corso (resta al centro della pedana)..."],
        ["Status_WeightCalibrating_Sitting"] = ["Weight calibrating (please gently rest your feet on the board)...", "体重キャリブレーション中(ボードに足をそっと乗せてください)...", "体重校准中(请轻轻将脚放在踏板上)...", "體重校準中(請輕輕將腳放在踏板上)...", "체중 보정 중(보드에 발을 살짝 올려주세요)...", "Étalonnage du poids (posez doucement les pieds sur la planche)...", "Gewichtskalibrierung läuft (bitte die Füße vorsichtig auf das Board stellen)...", "Calibrazione del peso in corso (appoggia delicatamente i piedi sulla pedana)..."],
        ...
        ["Devices_PopupTitle"] = ["Detected Bluetooth devices", "検知したBluetoothデバイス", "检测到的蓝牙设备", "偵測到的藍牙裝置", "감지된 블루투스 기기", "Appareils Bluetooth détectés", "Erkannte Bluetooth-Geräte", "Dispositivi Bluetooth rilevati"],
        ["Devices_PopupHeading"] = ["The following devices were detected:", "以下デバイスを検知しました", "检测到以下设备:", "偵測到以下裝置:", "다음 기기를 감지했습니다:", "Les appareils suivants ont été détectés :", "Folgende Geräte wurden erkannt:", "Sono stati rilevati i seguenti dispositivi:"],
        ["Devices_PopupNone"] = ["No devices were detected.", "デバイスは検知されませんでした", "未检测到任何设备。", "未偵測到任何裝置。", "감지된 기기가 없습니다.", "Aucun appareil détecté.", "Es wurden keine Geräte erkannt.", "Non è stato rilevato alcun dispositivo."],
        ["Button_OK"] = ["OK", "OK", "OK", "OK", "OK", "OK", "OK", "OK"],
        ["Caution_Title"] = ["Caution", "注意", "注意", "注意", "주의", "Attention", "Achtung", "Attenzione"],
        ["Caution_Message"] = [
            "- To prevent floor damage, please always use a mat while playing.\n- If you weigh 100kg (220 lb) or more, please do not jump.",
            "・床の破損を防ぐためプレイ時は必ず敷物をしてください\n・体重100kg以上の方はジャンプをしないでください",
            "・为防止地板损坏,游玩时请务必铺设垫子\n・体重100公斤以上者请勿跳跃",
            "・為防止地板損壞,遊玩時請務必鋪設墊子\n・體重100公斤以上者請勿跳躍",
            "・바닥 손상을 방지하기 위해 플레이 시 반드시 매트를 깔아주세요\n・체중 100kg 이상인 분은 점프하지 마세요",
            "- Pour éviter d'endommager le sol, utilisez toujours un tapis pendant le jeu.\n- Si vous pesez 100 kg ou plus, ne sautez pas.",
            "- Um Bodenschäden zu vermeiden, verwenden Sie beim Spielen immer eine Matte.\n- Wenn Sie 100 kg oder mehr wiegen, springen Sie bitte nicht.",
            "- Per evitare danni al pavimento, usa sempre un tappetino durante il gioco.\n- Se pesi 100 kg o più, non saltare.",
        ],
        ["MajorUpdateReset_Title"] = ["Settings reset", "設定リセット", "设置重置", "設定重設", "설정 재설정", "Réinitialisation des paramètres", "Einstellungen zurückgesetzt", "Ripristino impostazioni"],
        ["MajorUpdateReset_Message"] = ["A major update has been applied, so your settings have been reset to their defaults.", "メジャーアップデートがありましたので、設定のパラメータを一度リセットします", "由于进行了重大更新,设置参数将被重置一次", "由於進行了重大更新,設定參數將被重設一次", "주요 업데이트가 있었으므로 설정 값을 한 번 초기화합니다", "Une mise à jour majeure a été appliquée, vos paramètres ont donc été réinitialisés par défaut.", "Es wurde ein größeres Update durchgeführt, daher wurden deine Einstellungen einmalig zurückgesetzt.", "È stato applicato un aggiornamento importante, quindi le impostazioni sono state ripristinate ai valori predefiniti."],
        ...
        ["Settings_OutputMode_KeyboardMouseOscAuto"] = ["Keyboard+Mouse / OSC (automatic)", "キーボード+マウス/OSC自動切り替え", "键盘+鼠标/OSC(自动切换)", "鍵盤+滑鼠/OSC(自動切換)", "키보드+마우스/OSC(자동 전환)", "Clavier+souris / OSC (automatique)", "Tastatur+Maus / OSC (automatisch)", "Tastiera+mouse / OSC (automatico)"],
        ...
        ["Settings_WeightShiftSensitivity"] = ["Weight-shift mode strength", "体重移動モード強さ", "体重转移模式强度", "體重轉移模式強度", "체중 이동 모드 강도", "Intensité du mode transfert de poids", "Stärke des Gewichtsverlagerungs-Modus", "Intensità modalità spostamento peso"],
        ...
        ["Settings_ForcedControllerCorrectionHint"] = ["If your board's sensors respond extremely poorly, check the box below", "センサーの反応が極端に悪いボードの場合、以下チェックを入れてください", "如果踏板的传感器反应极差,请勾选下方选项", "如果踏板的感測器反應極差,請勾選下方選項", "보드의 센서 반응이 극도로 나쁜 경우 아래 체크박스를 선택하세요", "Si les capteurs de votre planche répondent très mal, cochez la case ci-dessous", "Wenn die Sensoren deines Boards extrem schlecht reagieren, aktiviere das Kästchen unten", "Se i sensori della tua pedana rispondono in modo estremamente scarso, seleziona la casella qui sotto"],
        ["Settings_ForcedControllerCorrection"] = ["Forced correction for poorly-responsive controllers", "反応の悪いコントローラーのための強制補正", "针对反应不佳控制器的强制校正", "針對反應不佳控制器的強制校正", "반응이 나쁜 컨트롤러를 위한 강제 보정", "Correction forcée pour les contrôleurs peu réactifs", "Erzwungene Korrektur für schlecht reagierende Controller", "Correzione forzata per controller poco reattivi"],
