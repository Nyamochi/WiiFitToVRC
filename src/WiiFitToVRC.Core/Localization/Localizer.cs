...
public static class Localizer
{
    ...
    private static readonly Dictionary<string, string[]> Table = new()
    {
        ...
        ["Settings_StepHold"] = ["Forward/backward key hold (ms)", "前進/後進のキー押下時間(ms)", "前进/后退按键保持时间(ms)", "前進/後退按鍵保持時間(ms)", "전진/후진 키 유지 시간(ms)", "Maintien de touche avant/arrière (ms)", "Tastenhaltezeit vor/zurück (ms)", "Durata pressione tasto avanti/indietro (ms)"],
