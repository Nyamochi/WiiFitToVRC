...
Wiiバランスボードを、VRChat(またはその他のWindowsアプリ)向けの歩行コントローラーに変えるアプリです。ボードに乗って体重を移動させるだけで、前進・後進・旋回・ジャンプ・しゃがみをキーボード/マウス入力、または仮想Xboxコントローラー入力に変換します。
...
docs/                      (現時点では英語のみ)
  ...
  VRCHAT_INPUT.md           素のSendInputがVRChatで効かない理由と、2つの対処法
```
