...
| 反应灵敏度(走路/冲刺/转向/跳跃/下蹲/步幅/行走冲刺持续判定/达到持续判定的步数) | 均可分别独立用"弱"~"强"(步幅、行走/冲刺持续判定为"窄"~"宽",达到持续判定的步数为1~5的步数)滑块调整(不影响前进/后退)。中间(默认值,达到持续判定的步数为3)保持原有判定基准不变。冲刺/转向/跳跃/下蹲这四项若将"弱"调到最小(0),则无论判定结果如何都完全不会输出按键 — 详见 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
