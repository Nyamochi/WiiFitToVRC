...
2. 双击下载的文件即可运行。
3. 应用启动后会自动开始搜索。只需按下 Wii 平衡板电池盒内的 **SYNC** 按钮即可自动连接(无需点击连接按钮)。由于规格限制,POWER 按钮无法用于此用途,因此每次连接都需要按 SYNC 按钮。
4. 连接完成后,校准也会自动开始。画面会显示“请将设备放在地板上”,5秒后自动开始校准,请在此之前把平衡板放在地板上并保持空载。
5. 校准完成后,站上平衡板像平常一样站立片刻即可完成准备。之后启动 VRChat,在平衡板上移动体重即可行走。
...
| 反应灵敏度(转向/跳跃/下蹲) | 三者可分别独立用"弱"~"强"滑块调整(不影响前进/后退)。中间(默认值)保持原有判定基准不变 |
