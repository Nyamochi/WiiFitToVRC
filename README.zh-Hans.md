...
- 按键绑定/手柄分配、转向速度、重量阈值、各类时间参数等均可在应用内设置窗口中精细调整。
- 主界面右下角实时显示行走距离(走路/冲刺分别以米为单位)和步数。仅统计从启动到退出的期间,不会保存。
- 多语言界面:自动检测 Windows 显示语言,内置日语、英语、简体/繁体中文、韩语、法语、德语、意大利语。
- **游玩姿势(站立/坐姿)切换** — 可在主界面左下角切换。选择坐姿后会自动调整触发所需的重量阈值和休眠/唤醒所需秒数,前进/后退、跳跃、下蹲的判定逻辑也会切换为适合坐姿的版本(详见 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md))。
- 启动时自动检查 GitHub 上的最新版本,如有更新会弹出通知(附更新内容和仓库链接,点击 OK 即可关闭,不会强制操作)。
...
| 反应灵敏度(走路/冲刺/转向/跳跃/下蹲/步幅/行走冲刺持续判定/达到持续判定的步数) | 均可分别独立用"迟钝"~"敏感"(步幅、行走/冲刺持续判定为"窄"~"宽",达到持续判定的步数为1~15的步数)滑块调整(不影响前进/后退)。中间(默认值,达到持续判定的步数为7)保持原有判定基准不变。冲刺/转向/跳跃/下蹲这四项若将"迟钝"调到最小(0),则无论判定结果如何都完全不会输出按键。转向不会持续,始终每次只动作一步 — 详见 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
| 转向方式 | 可在"长按"(向左右倾斜并保持)与"踏步"(对角面板交替踩踏,默认)之间切换转向判定逻辑 |
| 冲刺输入方式 | 可在键盘/键盘+鼠标模式下,将冲刺发送方式在"组合键"(修饰键+前进键,默认)与"按键连按两次"(按一次前进键后再长按)之间切换 |
| 转向速度 | 各输出方式的绝对值不同,因此每次切换输出方式单选按钮时,滑块的范围与数值都会自动切换。左右共用同一数值。选择“键盘(Q/E 转向)”“OSC 功能”时滑块会消失,显示“无设置项”(OSC 的转向是 `/input/LookLeft`・`/input/LookRight` 这种单纯的按钮按下,没有强度概念) |
| 转向长度 | 仅在 OSC/虚拟手柄输出下,一次转向操作实际持续输出的长度(默认1秒,窄~宽可调)。当 VRChat 的 OSC 输入等因转向脉冲过短而无反应时可以延长。两种输出方式共用同一数值,选择键盘/键盘+鼠标时该项不会显示 |
...
| 手柄标签页 | 虚拟手柄模式下各动作对应的按钮(转向速度请在“常规”标签页的“转向速度”中设置) |
