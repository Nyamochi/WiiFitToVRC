...
| 反应灵敏度(走路/冲刺/转向/跳跃/下蹲/步幅/行走冲刺持续判定/达到持续判定的步数) | 均可分别独立用"弱"~"强"(步幅、行走/冲刺持续判定为"窄"~"宽",达到持续判定的步数为1~10的步数)滑块调整(不影响前进/后退)。中间(默认值,达到持续判定的步数为5)保持原有判定基准不变。冲刺/转向/跳跃/下蹲这四项若将"弱"调到最小(0),则无论判定结果如何都完全不会输出按键。转向不会持续,始终每次只动作一步 — 详见 [docs/GESTURE_DETECTION.md](docs/GESTURE_DETECTION.md) |
| 转向方式 | 可在"长按"(向左右倾斜并保持)与"踏步"(对角面板交替踩踏,默认)之间切换转向判定逻辑 |
| 冲刺输入方式 | 可在键盘/键盘+鼠标模式下,将冲刺发送方式在"组合键"(修饰键+前进键,默认)与"按键连按两次"(按一次前进键后再长按)之间切换 |
| 转向速度 | 各输出方式的绝对值不同,因此每次切换输出方式单选按钮时,滑块的范围与数值都会自动切换。左右共用同一数值。选择“键盘(Q/E 转向)”时滑块会消失,显示“无设置项” |
